...
class AffinityDatabase(object):
    ...
    def db_commit(self, stmt=None):
        if stmt is not None:
            print (len(self.stmt_bucket))
            self.stmt_bucket.append(stmt)
        if stmt is None:
            print ("stmt none")
            print (len(self.stmt_bucket))
        if ( stmt is None and len(self.stmt_bucket)>0 ) or (len(self.stmt_bucket)>1000):
            print ('insert database')
            
            for stmt in self.stmt_bucket:
                self.conn.execute(stmt)
            self.conn.commit()
            self.stmt_bucket = []
    
    @lockit
    def insert(self, table_idx, values, head=None, bucket=False):
        for value in values:
            try:
                self.insert_or_replace(table_idx, [value], head, bucket)
            except Exception as e:
                print e
        

    def insert_or_replace(self, table_idx, values, head=None, bucket=False):

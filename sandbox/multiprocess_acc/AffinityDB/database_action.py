...
def download(table_idx, param, input_data):         # todo(maksym) input_data = pdb_ids
    '''
...
def split_ligand(table_idx, param, input_data, db):
    '''
...
def split_receptor(db,table_idx, param, datum):         # todo (maksym) param = params;
    print ('split receptor')
    try:                                             # todo (maksym) datum = pdb_name
        ...
        for chain, resnum, resname in ligands:
            try:
                ...
                records = [record]
                db.insert(table_idx, records,bucket=True)
            except Exception as e:
...
def reorder(table_idx, param, input_data):                                                      # todo(maksym) smina_reorder
    try:

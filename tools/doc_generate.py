...
<script>
    ...
    def stylish_cls_attr(self, attr_dict):
        styled_attr  = []
        for attr_name in sorted(attr_dict.keys()):
            styled_attr.append(self._wrap(self._wrap(self._wrap(attr_name,'code'),'b') + ' : ' + attr_dict[attr_name], 'li'))
        return self._wrap('\n'.join(styled_attr), 'ul')
        #return '<ul>' + '\n'.join(styled_attr) 

    def save_class_attr(self, clsobj, name_chain):
            """
            record attributes in class
            """

            cls_name = name_chain + '.' + clsobj.__name__
            attr_dict = {}
            for attr_name in dir(clsobj):
                if not attr_name.startswith('_'):
                    attr = getattr(clsobj,attr_name)
                    if not type(attr).__name__ in ['NoneType','instancemethod']:
                        attr_dict[attr_name] = type(attr).__name__

            self.cls_attr_dict[cls_name] = attr_dict
    ...
    def dump_html(self, path):
        ...
        for mod_key in sorted(self.mod_dict.keys()):
            ...
            for sub_key in sorted(pack_dict['class'].keys()):
                ...
                # if the class have attributes create a card for them
                attr_dict = self.cls_attr_dict[sub_key]
                if len(attr_dict.keys()):
                    styled_attr = self.stylish_cls_attr(attr_dict)
                    cls_attr_cont = self._wrap(styled_attr, 'div', cls='section')
                    temp_str += self._card('Attributes',cls_attr_cont)
                    temp_str += '\n'
        ...
        for sub_mod_name in dir(mod):
            ...
            if sub_mod_name in ['__init__'] or not sub_mod_name.startswith('_') and not sub_mod_name.startswith('im_'):
                ...
                if type(sub_mod).__name__ in ['classobj']:
                    self.save_class_attr(sub_mod, name_chain+mod.__name__)

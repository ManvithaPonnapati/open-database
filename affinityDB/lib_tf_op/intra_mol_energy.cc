...
REGISTER_OP("IntraMolEnergy")
    .Input("lignad: float32")
    .Input("lig_elem: int32")
    .Input("movable_mat: float32")
    .Output("energy: float32");

class IntraMolEnergyOp : public OpKernel {
 public:
  explicit IntraMolEnergyOp(OpKernelConstruction* context) : OpKernel(context) {}
  // Define const value 
  ...
  void Compute(OpKernelContext* context) override {
    ...
    const Tensor& movable_mat = context->input(2);   


    // get value of tensor
    auto lig_coords = ligand.matrix<float>();
    auto lig_elem = ligand_elem.flat<int>();
    auto movable_matrix = movable_mat.matrix<float>();
    ...
    for (int i = 0; i< ligand.shape().dim_size(0); i++){
      for (int j = 0; j< ligand.shape().dim_size(0);j++){
        if (movable_matrix(i,j) == 1){
            float dist = std::sqrt(std::pow(2, lig_coords(i,0)-lig_coords(j,0))+
                                   std::pow(2, lig_coords(i,1)-lig_coords(j,0))+
                                   std::pow(2, lig_coords(i,2)-lig_coords(j,2)));

            if (dist <= const_cutoff){
                float opt_dist = smina_atom_type::data[lig_elem(i)].ad_radius + smina_atom_type::data[lig_elem(j)].ad_radius;
                ...
                //hydrophobic
                this_energy = hydrophobic(dist, opt_dist, lig_elem(i), lig_elem(j),0.5, 1.5);
                weight_energy = this_energy * -0.035069;
                curl_energy = curl(weight_energy);
                energy += curl_energy;

                this_energy = non_dir_h_bond(dist, opt_dist, lig_elem(i), lig_elem(j),-0.7, 0);
...
REGISTER_KERNEL_BUILDER(Name("IntraMolEnergy").Device(DEVICE_CPU), IntraMolEnergy);
...
def split_pdb(split_pdb, cuttoff_dist = 8, min_rec_atom_num=100,
              min_lig_atom_num = 5, discard_h = True, init="split_pdb_init"):
    """ Iterates through every molecule of the ligand in the receptor. Selects atoms of the receptor (and any chemicals,
     but not water) within within the cutoff distance of any atoms of the ligand. Saves ligand + selection of atoms
     around the receptor's binding site into the folder.
    ...
    :param min_rec_atom_num: minimun number of atoms be saved as binding site
    ...
    for chain, resnum, resname in ligs:
        ...
        if lig_atom_num < min_lig_atom_num:
            raise Exception("Ligand atom num {} smaller than {} ".format(lig_atom_num, min_lig_atom_num))               # FIXME: this exits
        rec_atom_num = rec.numAtoms()
        if rec_atom_num < min_rec_atom_num:
            raise Exception("Receptor atom num {} smaller than {}".format(rec_atom_num, min_rec_atom_num))              # FIXME ?
